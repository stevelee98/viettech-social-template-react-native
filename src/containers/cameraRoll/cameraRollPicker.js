        ...
        if (index >= 0) {
            ...
            if (selected.length < maximum) {
                let videos = selected.filter(item => {
                    return (
                        Utils.getTypeResource(item.mime) == resourceType.VIDEO
                    );
                });
                if (
                    (numVideoSelected >= 1 || videos.length >= 1) &&
                    Utils.getTypeResource(image.mime) == resourceType.VIDEO
                ) {
                    showMessage(localizes('maxVideosLength'));
                } else {
                    selected.push(image);
                }
            } else {
                showMessage(localizes('maxImagesLength') + maximum);
            }
        }
    ...
    render() {
        ...
        return (
            <View
                style={[
                    styles.wrapper,
                    { marginBottom: -Constants.PADDING, backgroundColor },
                ]}
            >
                {flatListOrEmptyText}
            </View>
